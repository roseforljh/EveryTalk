...
class HistoryManager(
    ...
    private fun getConversationId(messages: List<Message>): String {
        return messages.firstOrNull()?.id?.split("_")?.get(0) ?: UUID.randomUUID().toString()
    }
    ...
    suspend fun deleteConversation(indexToDelete: Int) {
        ...
        val conversationToDelete = stateHolder._historicalConversations.value.getOrNull(indexToDelete)
        if (conversationToDelete == null) {
            Log.w(TAG_HM, "Invalid delete request: Index $indexToDelete out of bounds.")
            return
        }
        ...
        if (successfullyDeleted) {
            ...
            // Now, save the entire updated history list to persistence.
            // The DataPersistenceManager will handle the diffing and actual deletion in the DB.
            persistenceManager.saveChatHistory(stateHolder._historicalConversations.value)
            persistenceManager.saveLastOpenChat(emptyList())
    ...
    suspend fun clearAllHistory() {
        ...
        if (stateHolder._historicalConversations.value.isNotEmpty() || stateHolder._loadedHistoryIndex.value != null) {
            ...
            persistenceManager.clearAllChatHistory()
            persistenceManager.saveLastOpenChat(emptyList())
