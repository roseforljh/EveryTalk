...
import io.github.roseforljh.kuntalk.StateControler.ApiHandler
        ...
        viewModelScope.launch {
            if (wasCurrentlySelected) {
                ...
                if (newSelectedConfigAfterDeletion == null) {
                    stateHolder._snackbarMessage.emit("请添加或选择一个新的API 配置")
                } else {
    ...
    fun clearAllConfigs() {
        if (stateHolder._apiConfigs.value.isNotEmpty() || stateHolder._selectedApiConfig.value != null) {
            ...
            viewModelScope.launch {
                ...
                delay(250)
                stateHolder._snackbarMessage.emit("请添加一个API 配置")
            }
        } else {
