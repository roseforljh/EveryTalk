...
import io.github.roseforljh.kuntalk.data.DataClass.AppStreamEvent
import io.github.roseforljh.kuntalk.data.DataClass.ApiContentPart
import io.github.roseforljh.kuntalk.data.network.ApiClient
...
class ApiHandler(
    ...
    fun cancelCurrentApiJob(reason: String, isNewMessageSend: Boolean = false, removePlaceholder: Boolean = true) {
    ...
    fun streamChatResponse(
        ...
        val contextForLog = when (val lastUserMsg = requestBody.messages.lastOrNull {
            ...
            is io.github.roseforljh.kuntalk.data.DataClass.SimpleTextApiMessage -> lastUserMsg.content
            is io.github.roseforljh.kuntalk.data.DataClass.PartsApiMessage -> lastUserMsg.parts
                .filterIsInstance<ApiContentPart.Text>().joinToString(" ") { it.text }

            else -> null
        }?.take(30) ?: "N/A"

        cancelCurrentApiJob("开始新的流式传输，上下�? '$contextForLog'", isNewMessageSend = true)
            ...
            "error" -> {
                viewModelScope.launch {
                    updateMessageWithError(
                        ...
                        error = IOException("SSE Error"),
                        isFromSseEvent = true,
                        sseErrorMessage = appEvent.message ?: "从服务器收到未知错误"
                    )
                }
                if (stateHolder.reasoningCompleteMap[messageIdForLog] != true) {
                    stateHolder.reasoningCompleteMap[messageIdForLog] = true
        ...
        withContext(Dispatchers.Main.immediate) {
            ...
            if (idx != -1) {
                ...
                if (!msg.isError) {
                    ...
                    val finalErrorText = if (isFromSseEvent) {
                        sseErrorMessage
                    } else {
                        when (error) {
                    ...
                    val errorMsg = msg.copy(
                        text = existingContent + errorPrefix + ERROR_VISUAL_PREFIX + finalErrorText,
                        isError = true,
                        contentStarted = true,
