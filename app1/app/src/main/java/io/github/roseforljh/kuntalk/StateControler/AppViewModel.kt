...
import io.github.roseforljh.kuntalk.data.DataClass.WebSearchResult
...
import io.github.roseforljh.kuntalk.ui.screens.viewmodel.ConfigManager
import io.github.roseforljh.kuntalk.ui.screens.viewmodel.DataPersistenceManager
...
class AppViewModel(
    ...
    private val persistenceManager =
        DataPersistenceManager(application, dataSource, database, stateHolder, viewModelScope)
                ...
                if (stateHolder.messages[currentIndexToInspect].sender == Sender.AI) {
                    ...
                    if (stateHolder._currentStreamingAiMessageId.value == aiMessageToRemove.id) {
                        apiHandler.cancelCurrentApiJob(
                            "为消息'${originalUserMessageId.take(4)}' 重新生成回答，取消旧AI",
   ...
   fun updateConfig(config: ApiConfig) = configManager.updateConfig(config)
    ...
    fun deleteConfigGroup(apiKey: String, modalityType: io.github.roseforljh.kuntalk.data.DataClass.ModalityType) {
        viewModelScope.launch(Dispatchers.IO) {
