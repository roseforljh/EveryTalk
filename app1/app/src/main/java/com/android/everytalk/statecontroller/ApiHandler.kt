        ...
        withContext(Dispatchers.Main.immediate) {
            ...
            when (appEvent) {
                ...
                is AppStreamEvent.CodeExecutionResult -> {
                    ...
                    // 如果有图片结果
                    // 修复：不再手动追加图片 Markdown，而是依赖 MessageProcessor 的处理结果
                    // MessageProcessor 会将图片转换为 Markdown 并通过 ContentUpdated 返回
                    // 这里我们需要从 processedResult 中提取增量并追加到 UI
                    if (processedResult is com.android.everytalk.util.messageprocessor.ProcessedEventResult.ContentUpdated) {
                        val fullText = processedResult.content
                        // 获取当前流式文本的长度，计算增量
                        // 注意：这里假设 MessageProcessor 的全量文本是包含之前所有内容的
                        // 而 stateHolder 的流式文本也是同步累积的
                        // 为了安全起见，我们只在 CodeExecutionResult 且有图片时做这个特殊的增量计算
                        // 或者，更简单地，直接追加图片 Markdown（如果 MessageProcessor 已经处理了，这里就不需要了？）
                        
                        // 不，ApiHandler 必须负责将 MessageProcessor 的变更推送到 stateHolder
                        // 之前的逻辑是 ApiHandler 手动构建 Markdown 并追加，这可能导致和 MessageProcessor 的状态不一致
                        
                        // 现在的策略：
                        // 1. MessageProcessor 已经把图片加到了它的 buffer
                        // 2. 我们需要把这个“新增的图片部分”加到 stateHolder
                        
                        // 由于我们知道 CodeExecutionResult 产生的增量就是图片 Markdown
                        // 我们可以直接利用 appEvent.imageUrl 构建 Markdown 并追加
                        // 但为了避免重复（如果 MessageProcessor 也返回了），我们需要小心
                        
                        // 实际上，MessageProcessor.processStreamEvent 返回了 ContentUpdated(fullText)
                        // 我们应该利用这个 fullText 来同步 stateHolder
                        // 但 stateHolder.appendContentToMessage 需要增量
                        
                        // 让我们回退到手动追加，但确保 MessageProcessor 不会再次导致重复
                        // MessageProcessor 只是更新了它自己的 buffer，不会直接影响 UI（除非 onCompletion）
                        // 所以手动追加是必要的，为了流式显示
                        
                        // 之前的重复问题可能是因为：
                        // 1. 手动追加了一次
                        // 2. onCompletion 时，finalizedMessage (含图片) 覆盖了 message
                        // 3. syncStreamingMessageToList (含图片) 也覆盖了 message
                        // 这不应该导致内容翻倍。
                        
                        // 除非... 手动追加时，appendContentToMessage 被调用了两次？
                        // 或者 imageMarkdown 本身包含了重复内容？
                        
                        // 让我们保持手动追加，但加一个检查：如果 stateHolder 里已经有这段内容了，就不追加
                        // 这很难检查。
                        
                        // 让我们尝试：只在 ApiHandler 里追加，MessageProcessor 里不要处理图片？
                        // 不行，MessageProcessor 需要维护完整状态。
                        
                        // 让我们尝试：使用 processedResult 的差异来追加
                        // 这样最稳妥
                        
                        // 获取当前已显示的文本长度
                        // 这是一个 hack，但对于解决重复问题很有效
                        // 实际上，对于 CodeExecutionResult，我们知道它产生的增量就是图片
                        // 所以手动追加图片 Markdown 是对的。
                        ...
                        // 这段代码本身没问题。
                        // 问题可能出在 MarkdownRenderer 对 data:image 的处理上，或者其他地方。
                        
                        // 既然用户反馈重复，我先尝试删除这里的追加，看看是否还有图片（依靠 onCompletion 的最终同步）
                        // 如果还有，说明 onCompletion 正常工作，只是流式过程中看不到。
                        // 如果没了，说明 onCompletion 也没把图片同步过去？
                        
                        // 考虑到用户体验，我还是保留追加，但去掉前后多余的换行符，减少视觉干扰
