...
class ComputerUiAlignmentRulesTest {
    ...
    fun `服务器选择浮层使用自适应紧凑名称卡且隐藏连接信息`() {
        ...
        val clipIndex = source.indexOf(".clip(shape)")
        val clickableIndex = source.indexOf(".combinedClickable(")
        assertTrue("按压反馈必须由圆角形状裁剪", clipIndex >= 0 && clickableIndex > clipIndex)
    ...
    fun `主机命令确认卡固定在输入框上方并只提供本次允许和拒绝`() {
        ...
        val chatScreenSource = sourceFile("ui/screens/MainScreen/chat/ChatScreen.kt")
            .readText(Charsets.UTF_8)
        val hostCardScrollSource = chatScreenSource
            .substringAfter("val scrollStateManager = rememberChatScrollStateManager")
            .substringBefore("LaunchedEffect(scrollSessionKey, historyLoadGeneration)")
        ...
        assertFalse(chatScreenSource.contains("wasAtBottomBeforeHostCommand"))
        assertFalse(hostCardScrollSource.contains("pinToRealBottomUntilUserScroll"))
        assertTrue(inputSource.contains("popupPositionProvider = hostCommandPopupPositionProvider"))
        ...
        assertTrue(panelSource.contains("AppFloatingCardScaffoldPopup("))
        assertTrue(panelSource.contains("header = {"))
        assertTrue(panelSource.contains("content = {"))
        assertTrue(panelSource.contains("footer = {"))
        val confirmationSource = panelSource
            .substringAfter("internal fun ComputerHostCommandConfirmationCard(")
            .substringBefore("internal fun computerStatusLabelRes")
        val headerSource = confirmationSource.substringAfter("header = {")
            .substringBefore("content = {")
        val footerSource = panelSource.substringAfter("footer = {")
            .substringBefore("internal fun computerStatusLabelRes")
        assertTrue("权限卡标题区必须覆盖为卡片背景", headerSource.contains(".background(cardBackground)"))
        assertTrue("权限卡底部操作区必须覆盖为卡片背景", footerSource.contains(".background(cardBackground)"))
        assertTrue("权限卡拒绝按钮不得使用 Material 默认底色", footerSource.contains("containerColor = Color.Transparent"))
