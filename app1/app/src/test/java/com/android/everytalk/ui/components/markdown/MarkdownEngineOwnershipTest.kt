...
class MarkdownEngineOwnershipTest {
    ...
    fun `Markdown标准组件使用MikePenz并集中配置移动端视觉层级`() {
        ...
        assertTrue(adapter.contains("color = MaterialTheme.colorScheme.primary"))
        assertTrue(adapter.contains("textDecoration = TextDecoration.Underline"))
        ...
        assertTrue(adapter.contains("MarkdownInlineImageWithFailure(model)"))
