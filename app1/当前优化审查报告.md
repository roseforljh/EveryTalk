...
- Gradle、Wrapper、版本目录、仓库顺序、CI 发版 workflow。
- Android Manifest、外部分享 Intent、FileProvider、更新检查、Release 签名链路。
- OpenAI Compatible、OpenAI Responses、Gemini、MCP Streamable HTTP、SSE 解析。
...
- 修复：`exportSchema=true`，新增 `room.schemaLocation`，生成 `5.json` 与 `6.json`，新增 5->6 迁移测试。
